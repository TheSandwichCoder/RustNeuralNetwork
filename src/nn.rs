...
impl Perceptron{
    pub fn new(n_inputs: usize, is_activation: bool) -> Perceptron{
        let mut perc = Perceptron{n_inputs: n_inputs, weights: Vec::new(), bias: 0.0, is_activation: is_activation};
    ...
    pub fn backward(&mut self, inputs: &Vec<f32>, z: f32, part_deriv: f32){
        ...
        for input_i in 0..self.n_inputs{
            let gradient = inputs[input_i] * part_deriv * s;
            
            self.weights[input_i] += gradient * lr;
        }

        self.bias += s * part_deriv * lr;
    }
